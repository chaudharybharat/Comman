...
public class CustomeGraphSleep  extends SeekBar implements View.OnTouchListener,View.OnClickListener {
    ...
    public CustomeGraphSleep(Context context) {
        super(context);
        //   mSeekBar_data = new ArrayList<ProgressItem>();
        mColorList = new ArrayList<Integer>();
    }

    public CustomeGraphSleep(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CustomeGraphSleep(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      /*  if (mSeekBar_data.size() > 0) {*/
        ...
        canvas.drawText("24", 0, 2, progressItemRight - layout_widh, 100, paint_txt_normal);
        ...
        for (int j = 0; j < mSeekBar_data.size(); j++) {
            ...
            if( mSeekBar_data.get(j).getValue()<=24) {

                float value = mSeekBar_data.get(j).getValue();

                Log.e("test", "====>>" + value);

                float x_point = value * totla_with_data / 24;
...
*/
        ...
        if (pos != -1) {
            ...
            if(pos<mSeekBar_data.size()){
                ...
                float value_with = paint_txt_normal.measureText(heart_value);
...
*/
                //point value display
                canvas.drawText(heart_value, 0, heart_value.length(), pathListText.get(pos).getX()-value_with/2, pathListText.get(pos).getY(), paint_line);
                canvas.drawLine(pathListLinestart.get(pos).getX(), pathListLinestart.get(pos).getY(), pathListLineend.get(pos).getX(), pathListLineend.get(pos).getY(), paint_line);
