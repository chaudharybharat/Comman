...
public class CustomGraphPositivity extends SeekBar implements View.OnTouchListener,View.OnClickListener {
    ...
    public CustomGraphPositivity(Context context) {
        super(context);
        //   mSeekBar_data = new ArrayList<ProgressItem>();
        mColorList = new ArrayList<Integer>();
    }

    public CustomGraphPositivity(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CustomGraphPositivity(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        layout_widh = getResources().getDimensionPixelSize(R.dimen.dp_20);
      ...
      /*  if (mSeekBar_data.size() > 0) {*/
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i == 0) {
                canvas.drawText("0", 0, 1, lastProgressX, 100, paint_txt_normal);
            }
            ...
            if(i<3){
                ...
                //canvas.drawLine(lastProgressX + total_with, 60, lastProgressX + total_with, progressBarHeight + 30, paint_line);
            }
            //round shape
        ...
        canvas.drawText("100", 0, 3, progressItemRight - layout_widh, 100, paint_txt_normal);
        ...
        for (int j = 0; j < mSeekBar_data.size(); j++) {
            float value = mSeekBar_data.get(j).getValue();

            float x_point = value * totla_with_data / 100;
            ...
            float width = paint_you.measureText(str_value) + 5;
            float width_network = paint_network.measureText(str_value) + 5;
          ...
          //  if (mSeekBar_data.get(j).getValue() >= 45) {
                ...
                paint_text.getTextBounds(str_value, 0, str_value.length(), bounds1);
            int text_height = bounds1.height()+5;
            ...
            if(j==0) {
               //50
                if( value<20){
                    ...
                    //second postion
                    // right side
                    canvas.drawText(str_value, 0, str_value.length(), postion_x+5, getResources().getDimensionPixelSize(R.dimen.txt_height_data), paint_you);
                    listtextXy.add(new ClassXY(postion_x+5,postion_x+width));

                }else {
                    //top text
                    pathListText.add(new TextViewxyPos(postion_x ,  getResources().getDimensionPixelSize(R.dimen.margin_sp)));
                    Log.e("test","=>>>> width_you"+width);
                    //leftSide
                    canvas.drawText(str_value, 0, str_value.length(), postion_x - width,getResources().getDimensionPixelSize(R.dimen.txt_height_data), paint_you);
                    listtextXy.add(new ClassXY(postion_x - width,postion_x));
                }
                ...
                if( value<30){
                    ...
                    //second postion
                    // right side
                    canvas.drawText(str_value, 0, str_value.length(), postion_x, getResources().getDimensionPixelSize(R.dimen.txt_height_data_network), paint_network);
                    ...
                    pathListText.add(new TextViewxyPos(postion_x ,  getResources().getDimensionPixelSize(R.dimen.margin_sp)));
                    //leftSide
                    canvas.drawText(str_value, 0, str_value.length(), postion_x - width_network,getResources().getDimensionPixelSize(R.dimen.txt_height_data_network), paint_network);
                    listtextXy.add(new ClassXY(postion_x - width_network,postion_x));

                }
               /* canvas.drawText(str_value, 0, str_value.length(), postion_x - width, bottom_end_point * value_incere, paint_text);
                listtextXy.add(new ClassXY(postion_x - width,postion_x));

            }else {
                Log.e("test","callllllll............=> ");
                listtextXy.add(new ClassXY(postion_x - width/2,postion_x+ width/2));
                canvas.drawText(str_value, 0, str_value.length(), postion_x - width/2, (bottom_end_point * value_incere)+text_height, paint_text);
            */
        ...
        if (pos != -1) {
            ...
            if(pos<mSeekBar_data.size()){
                ...
                Log.e("test","Path==>>"+pathListText.get(pos).getY());
                Log.e("test","Path second==>>"+getResources().getDimensionPixelSize(R.dimen.margin_sp));
