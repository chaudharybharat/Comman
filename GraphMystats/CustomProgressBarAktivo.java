...
public class CustomProgressBarAktivo extends SeekBar implements View.OnTouchListener,View.OnClickListener {
    ...
    int pos = -1;
  int bottom_end_point=getResources().getDimensionPixelSize(R.dimen.px_200);
    public CustomProgressBarAktivo(Context context) {
        super(context);
     //   mSeekBar_data = new ArrayList<ProgressItem>();
        mColorList = new ArrayList<Integer>();
    }

    public CustomProgressBarAktivo(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CustomProgressBarAktivo(Context context, AttributeSet attrs, int defStyle) {
 ...
 public void initColorData(ArrayList<Integer> progressItemsList) {
    ...
    protected void onDraw(Canvas canvas) {
        bottom_end_point= getHeight()/2;
        Log.e("test","bottom_end_point"+bottom_end_point);
        ...
        layout_widh = getResources().getDimensionPixelSize(R.dimen.dp_10);
        you_size = getResources().getDimensionPixelSize(R.dimen.you);
        paint_txt_normal.setTextSize(scaledSize);

        Typeface fromAsset = Typeface.createFromAsset(getContext().getAssets(), "fonts/Montserrat-Light.ttf");
        ...
        paint_txt_normal.setTypeface(fromAsset);
      ...
      /*  if (mSeekBar_data.size() > 0) {*/
        ...
        //right left padding getWidthh /7 is value set
           ...
           // int progressBarHeight = getResources().getDimensionPixelSize(R.dimen.margin_top);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (i == 0) {
                    canvas.drawText("45", 0, 2, lastProgressX, getResources().getDimensionPixelSize(R.dimen.top_maring), paint_txt_normal);
                }
               ...
               /* if (i == mSeekBar_data.size() - 1 && progressItemRight != progressBarWidth) {
                ...
                if(i<3){
                    ...
                    //canvas.drawLine(lastProgressX + total_with, getResources().getDimensionPixelSize(R.dimen.top), lastProgressX + total_with, progressBarHeight + getResources().getDimensionPixelSize(R.dimen.bootm), paint_line);
                }
                //round shape
            ...
            canvas.drawText("95", 0, 2, progressItemRight - layout_widh, getResources().getDimensionPixelSize(R.dimen.top_maring), paint_txt_normal);
          //main_point2=main_point2-45;
            //Text
            RectF rect = new RectF(lastProgressX, 20, lastProgressX, 400);
            paint_text.setColor(Color.BLACK);
            paint_text.setTextSize(convertPixelsToDp(50f,getContext()));
            ...
            for (int j = 0; j < mSeekBar_data.size(); j++) {
                ...
                float value = mSeekBar_data.get(j).getValue() - 45;

                float x_point = value * totla_with_data / 50;
              ...
              //  float width = paint_text.measureText(str_data)+5;
                Rect bounds = new Rect();
                float width = paint_text.measureText(str_data)+5;

               // paint_text.setTextSize(convertPixelsToDp(30f,getContext()));// have this the same as your text size


                //paint_text.getTextBounds(str_data, 0, str_data.length(), bounds);
              ...
              //  width =  bounds.width()+5;
                ...
                if(mSeekBar_data.get(j).getValue() >= 45){
                    ...
                    Log.e("test", "test=>>" + mSeekBar_data.get(j).getValue());
                    Log.e("test", "value_incere=>>" + value_incere);
                    ...
                    String str_value = mSeekBar_data.get(j).getTitle();
                    float width_you = paint_you.measureText(""+str_value)+5;
                   ...
                   if(j==0){
                     //25
                       if( value<10){
                           ...
                           listtextXy.add(new ClassXY(postion_x+5,postion_x+width_you));

                       }else {
                           //top text
                           pathListText.add(new TextViewxyPos(postion_x ,  getResources().getDimensionPixelSize(R.dimen.margin_sp)));
                           //leftSide
                           Log.e("test","=>>>> width_you"+width_you);
                           canvas.drawText(str_data, 0, str_data.length(), postion_x - width_you,getResources().getDimensionPixelSize(R.dimen.txt_height_data), paint_you);
                           listtextXy.add(new ClassXY(postion_x - width_you,postion_x));
                       }
                    ...
                    canvas.drawCircle(postion_x, getResources().getDimensionPixelSize(R.dimen.txt_height_data), 5f, paint);
                       ...
                       if (value <15) {
                           ...
                           //leftSide
                          /* canvas.drawText(str_data, 0, str_data.length(), postion_x - width_network,bottom_end_point * value_incere, paint_network);
                           listtextXy.add(new ClassXY(postion_x - width_network,postion_x));
*/
                           ...
                           //second postion
                           // right side
                           //   canvas.drawText(str_data, 0, str_data.length(), postion_x, bottom_end_point * value_incere, paint_txt_normal);

                           // dont point below center
                  ...
                  /* dhdhdh
                   canvas.drawLine(postion_x,progressBarHeight+20, postion_x, bottom_end_point * value_incere, paint_line);
                    ...
                    Log.e("test","bottom_end_point last"+bottom_end_point);
...
*/
    ...
    public float getIntToPx(int value){
