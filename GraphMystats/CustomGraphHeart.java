...
public class CustomGraphHeart  extends SeekBar implements View.OnTouchListener,View.OnClickListener {
    ...
    public CustomGraphHeart(Context context) {
        super(context);
        //   mSeekBar_data = new ArrayList<ProgressItem>();
        mColorList = new ArrayList<Integer>();
    }

    public CustomGraphHeart(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CustomGraphHeart(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      /*  if (mSeekBar_data.size() > 0) {*/
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i == 0) {
                canvas.drawText("40", 0, 2, lastProgressX, 100, paint_txt_normal);
            }
        ...
        canvas.drawText("120", 0, 3, progressItemRight - layout_widh, 100, paint_txt_normal);
        ...
        for (int j = 0; j < mSeekBar_data.size(); j++) {
            ...
            float value = mSeekBar_data.get(j).getValue() - 40;

                    Log.e("test","====>>"+value);

            float x_point = value * totla_with_data / 80;
