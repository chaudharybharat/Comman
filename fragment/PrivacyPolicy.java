...
public class PrivacyPolicy extends BaseFragment implements View.OnClickListener {
    ...
    @BindView(R.id.tv_pricancy_policy)
    TextView tv_pricancy_policy;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        tv_title.setText(CommonKeys.PRIVACY_POLICY);
        tv_back.setTypeface(common_methods.getFont(mActivity,CommonKeys.Montserrat_Bold));
        tv_pricancy_policy.setTypeface(common_methods.getFont(mActivity,CommonKeys.Montserrat_ExtraLight));
        tv_back.setTypeface(common_methods.getFont(mActivity,CommonKeys.Montserrat_Bold));
        intiComponet();
        mActivity.tagmanager("Privacy Policy Screen","signin_privacy_screen_view");
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            llMainScroll.setOnScrollChangeListener(new View.OnScrollChangeListener() {
                ...
                public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                    if (flag == false){
                        mActivity.tagmanager("Privacy Policy Screen","signin_privacy_screen_scroll");
                        flag = true;
                        ...
                        if(response.body().getData()!=null){
                            ...
                            if(data!=null && data.size()>0){
                                for (int i = 0; i <data.size() ; i++) {
                                    data.get(i).save();
                                    if(data.get(i).getCode().equalsIgnoreCase(PRIVACY_POLICY)){
                                     String pricvay_policy=data.get(i).getDescription();
                                     if(Validation.isRequiredField(pricvay_policy)){
                                         tv_pricancy_policy.setText(Html.fromHtml(pricvay_policy));
    ...
    private void setHeader() {
        ...
        if(bg_image!=null){
            if (Validation.isRequiredField(bg_image.getPrivacy_policy_background_image())) {
                mActivity.setBackgroudnImage(bg_image.getPrivacy_policy_background_image());
            }
        }
    ...
    private void intiComponet() {
        ...
        if(postCMS!=null && postCMS.size()>0){
            for (int i = 0; i <postCMS.size() ; i++) {
                if(postCMS.get(i).getCode().equalsIgnoreCase(PRIVACY_POLICY)){
