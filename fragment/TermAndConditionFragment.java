 ...
 */
public class TermAndConditionFragment extends BaseFragment implements View.OnClickListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        setHeader();
        initComponet();
        mActivity.tagmanager("Terms & Conditions Screen", "signin_tc_screen_view");
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            llMainScroll.setOnScrollChangeListener(new View.OnScrollChangeListener() {
                ...
                public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                    if (flag == false) {
                        mActivity.tagmanager("Terms & Conditions Screen scroll", "signin_tc_screen_scroll");
                        flag = true;
                    }
    ...
    private void setHeader() {
        ...
        if (bg_image != null) {
            if (Validation.isRequiredField(bg_image.getTerms_and_conditions_background_image())) {
                mActivity.setBackgroudnImage(bg_image.getTerms_and_conditions_background_image());
            }
        }
    ...
    private void initComponet() {
        ...
        if (postCMS != null && postCMS.size() > 0) {
            ...
            for (int i = 0; i < postCMS.size(); i++) {
                if (postCMS.get(i).getCode().equalsIgnoreCase(TERMS_AND_CONDITION)) {
                        ...
                        if (response.body().getData() != null) {
                            ...
                            if (data != null && data.size() > 0) {
                                for (int i = 0; i < data.size(); i++) {
                                    data.get(i).save();
                                    if (data.get(i).getCode().equalsIgnoreCase(TERMS_AND_CONDITION)) {
