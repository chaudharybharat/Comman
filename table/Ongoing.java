...
@Table(name = Ongoing.TABLE_NAME, database = Aktivodb.class)
public class Ongoing extends BaseModel{
    ...
    public static List<Ongoing> getOngoingList(){
        List<Ongoing>OngoingList = SQLite.select().distinct().from(Ongoing.class).queryList();
        if(OngoingList!=null){
            return OngoingList;
        }
        return null;
    }
    public static Ongoing getOngoingDetail(String id){
        Ongoing ongoing = SQLite.select().distinct().from(Ongoing.class).where(Ongoing_Table._id.eq(id)).querySingle();
        if(ongoing!=null){
            return ongoing;
        }
        return null;
