...
@Table(name = Almost_over.TABLE_NAME, database = Aktivodb.class)
public class Almost_over extends BaseModel{

    public static final String TABLE_NAME = "almost_over";
    ...
    public static List<Almost_over> getAlmostOverList(){
        List<Almost_over> almostOverList = SQLite.select().distinct().from(Almost_over.class).queryList();
    ...
    public static Almost_over getAlmost_overDetail(String id){
        Almost_over almost_over = SQLite.select().distinct().from(Almost_over.class).where(Almost_over_Table._id.eq(id)).querySingle();
        if(almost_over!=null){
            return almost_over;
        }
        return null;
